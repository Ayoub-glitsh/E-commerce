...
const useCartStore = create((set) => ({
    // 1. On ajoute la gestion des commandes depuis le localStorage
    orders: (() => {
    ...
    // 2. On ajoute la fonction pour créer une commande
    ...
    handleWish: (product) => set((state) => {
        ...
        if (wished) {
            ...
            return { wish: state.wish.filter((item) => item.id !== product.id) };
        } else {
    ...
    deleteFromWishList: (id) => set((state) => {
        ...
        return { wish: state.wish.filter((item) => item.id !== id) };
    }),
    ...
    clearCart: () => set({ cart: [] }),
    addProductToCart: (product, qty = 1) => set((state) => {
